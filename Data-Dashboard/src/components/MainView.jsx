...
const MainView = () => {
  ...
  async function fetchData() {
    const response = await fetch(
      "https://api.seatgeek.com/2/events?per_page=1000&client_id=MzI4NTMyMjl8MTY4MDYwNDc3NS42NDAxNzk2"
    );
  ...
  useEffect(() => {
    ...
    if (stateFilter != "") {
      ...
      setFilteredEvents((prevEvents) =>
        prevEvents.filter((event) => event.venue.state === stateFilter)
      );
    } else {
    ...
    if (selectedDate != "") {
      setFilteredEvents((prevEvents) =>
        prevEvents.filter(
          (event) =>
            new Date(event.datetime_local).toISOString().substring(0, 10) ==
            selectedDate
        )
      );
    }
  ...
  return (
    <div className="mainContainer">
      <div className="container1">
        <h1 className="header">Happening Hub</h1>
        <hr />
        ...
        <div className="filter">
          ...
          <p>
            ...
            <select onChange={handleCapacity} id="capacity">
              <option value="">All</option>
          ...
          {filteredEvents.map((event) => (
            <div className="card">
              <p key={event.id}>
                ...
                <Link to={`/detail/${event.id}`} className="details">
                  🔗
                </Link>
              </p>
              <p>
                ...
                Start Time (UTC):{" "}
                <span>{new Date(event.datetime_utc).toLocaleString()},</span>
                End Time (UTC):{" "}
                <span>
                  {event.enddatetime_utc
                    ? new Date(event.enddatetime_utc).toLocaleString()
                    : "N/A"}
                </span>
              </p>
              <p>
                ...
                Capacity:
                <span>
                  {event.venue.capacity > 0 ? event.venue.capacity : "N/A"}
                </span>
              </p>
            </div>
          ))}
